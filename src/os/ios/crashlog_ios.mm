...
/** @file crashlog_ios.mm iOS crash log handler. */
...
/**
 * iOS implementation for the crash logger.
 */
class CrashLogIOS : public CrashLog {
	...
	{
		CrashLogIOS *crashlog = reinterpret_cast<CrashLogIOS *>(context);
...
public:
	 ...
	 */
	CrashLogIOS(int signum) : signum(signum) {}
	...
	void DisplayCrashDialog() const
	{
		...
		ShowIOSDialog(crash_title, message, "Quit");
	}
	...
	static CrashLogIOS *current;
};

/* static */ CrashLogIOS *CrashLogIOS::current = nullptr;
...
{
	if (CrashLogIOS::current == nullptr || !CrashLogIOS::current->try_execute_active) {
		fmt::print("Something went seriously wrong when creating the crash log. Aborting.\n");
		_exit(1);
	}

	longjmp(CrashLogIOS::current->internal_fault_jmp_buf, 1);
}
...
{
	if (CrashLogIOS::current != nullptr) {
	...
	if (_gamelog.TestEmergency()) {
		ShowIOSDialog("A serious fault condition occurred in the game. The game will shut down.",
	...
	CrashLogIOS *log = new CrashLogIOS(signum);
	CrashLogIOS::current = log;
	log->MakeCrashLog();
